...
class simple_disk:
    """
    A class to build simple models of molecular line emission from
    protoplanetary disks. There are five main aspects that fully describe the
    model:
    ...
    Args:
        ...
        dVturb (Optional[float]): Non-thermal line-width fraction (of sound
    ...
    def __init__(self, inc, PA, x0=0., y0=0., dist=100., mstar=1.,
                 ...
                 tau0=100., tauq=0.0, taueta=np.inf, taumax=100.0,
                 dV0=None, dVq=None, dVmax=1000., dVturb=0.,
        ...
        self.dV0, self.dVq, self.dVmax, self.dVturb = dV0, dVq, dVmax, dVturb
    ...
    def _populate_coordinates(self):
        ...
        # These are all in units of [arcsec].
        ...
        # These are all in units of [au]. The cylindrical coordinates are in
        # units of [au], [radians].
        ...
        # Define two sets of cylindrical coordintes, one for each of the two
        # emission surfaces. If `z0 = 0.0` then the two emission surfaces are
        # equal and represent the miplane coordinates. The units are [au],
        # [radians] and [au] for (r, t, z), respectively.
    ...
    def _set_linewidth(self):
        ...
        """
        if self.dV0 is None:
            self.dV_f = np.sqrt(2 * self.mu / self.mu_l + self.dVturb**2)
            self.dV_f = np.clip(self.cs_f * self.dV_f, 0.0, self.dVmax)
            if self._flat_disk:
                self.dV_b = None
            else:
                self.dV_b = np.sqrt(2 * self.mu / self.mu_l + self.dVturb**2)
                self.dV_b = np.clip(self.cs_b * self.dV_b, 0.0, self.dVmax)
        else:
            if self.dVq is None:
    ...
    def _set_brightness(self):
        """
        Sets the brightness profile in [K]. The radial profile is assumed to be
        an exoponentially tapered power-law.
        """
        self.Tb_f = self.Tb0 * (self.r_sky_f / self.r0)**(-self.Tbq)
        self.Tb_f *= np.exp(-(self.r_sky_f / self.r_l)**self.Tbeps)
        ...
        if self._flat_disk:
            ...
            self.Tb_b *= np.exp(-(self.r_sky_f / self.r_l)**self.Tbeps)
    ...
    def cs_f(self):
        """
        Returns the gas sound speed for the front side of the disk in [m/s].
        """
        return np.sqrt(sc.k * self.Tb_f / self.mu / self.mH)

    @property
    def cs_b(self):
        """
        Returns the gas sound speed for the back side of the disk in [m/s].
        """
        return np.sqrt(sc.k * self.Tb_b / self.mu / self.mH)
    ...
    def _set_tau(self):
        """
        Sets the tau radial profile. The radial profile is assumed to be an
        exponentially tapered power-law.
        """
        self.tau = self.tau0 * (self.r_sky_f / self.r0)**self.tauq
        self.tau *= np.exp(-(self.r_sky_f / self.r_l)**self.taueta)
    ...
    def Tb_disk(self):
        """
        Disk-frame brightness temperature distribution.
        """
        Tb = self.Tb0 * (self.r_disk / self.r0)**(-self.Tbq)
        Tb *= np.exp(-(self.r_disk / self.r_l)**self.Tbeps)
        return np.where(self._in_disk, Tb, np.nan)

    @property
    def cs_disk(self):
        """
        Disk-frame sound speed.
        """
        return np.sqrt(sc.k * self.Tb_disk / self.mu / self.mH)
    ...
    def dV_disk(self):
        ...
        """
        if self.dV0 is None:
            dV = self.cs_disk * np.sqrt(2 * self.mu / self.mu_l + self.dVturb**2)
        else:
            if self.dVq is None:
                self.dVq = 0.5 * self.Tbq
    ...
    def _z_func(self, r):
        ...
        z *= np.exp(-(r * self.dist / self.r_l)**self.zphi) / self.dist
    ...
    @njit
    def get_pure_cube(self, velax, v0, dV, Tb_f, Tb_b=None):
        """
        Generate the image cube without noise.
        """
    ...
    def plot_keplerian(self, ax=None, logy=True, top_axis=True,
                       return_fig=False):
        """
        Plot the Keplerian rotation profile.

        Args:
            ax (Optional[matplotlib axis]): Axis used for plotting.
            logy (Optional[bool]): Use a logarithmic scale for the y-axis.
            top_axis (Optional[bool]): Include a top axis showing the radius in
                [au] to compliment the radius in [arcsec] on the bottom axis.
    ...
    def plot_linewidth(self, ax=None, top_axis=True, return_fig=False):
        """
    ...
    def plot_brightness(self, ax=None, top_axis=True, return_fig=False):
        """
    ...
    def plot_tau(self, ax=None, top_axis=True, return_fig=False):
        """
    ...
    def plot_emission_surface(self, ax=None, top_axis=True, return_fig=False):
        """
    ...
    def plot_radii(self, ax, rvals, contour_kwargs=None, projection='sky',
        ...
        Args:
            ax (matplotlib axis):
            rvals (list): List of radii to overplot in [au].
            contour_kwargs (Optional[dict]): Plotting kwargs to pass to
                ``matplotlib.contour``.
            projection (Optional[str]): The projection used for plotting,
                either ``'sky'`` or ``'disk'``.
            side (Optional[str]): If ``projection='sky'``, specify the side of
                the disk to plot, either ``'front'`` or ``'back'``.
        """
