...
class simple_disk:
    ...
    def _get_midplane_cart_coords(self, x0, y0, inc, PA):
        ...
        """
        x_sky, y_sky = self._get_cart_sky_coords(x0, y0)
        x_rot, y_rot = simple_disk._get_rotated_coords(x_sky, y_sky, PA)
        return simple_disk._get_deprojected_coords(x_rot, y_rot, inc)
    ...
    def set_Tb_profile(self, function, side='both', min=0.0, max=None):
        """
        ...
        """
        if side.lower() == 'both':
            self.Tb_f = np.clip(function(self.r_disk_f), a_min=min, a_max=max)
            self.Tb_b = np.clip(function(self.r_disk_b), a_min=min, a_max=max)
        elif side.lower() == 'front':
            self.Tb_f = np.clip(function(self.r_disk_f), a_min=min, a_max=max)
        elif side.lower() == 'back':
            self.Tb_b = np.clip(function(self.r_disk_b), a_min=min, a_max=max)
    ...
    def get_cube(self, velax, vlsr=0.0):
        ...
        tau_f = gaussian(velax[:, None, None],
    ...
    def plot_Tb_profile(self, side='front', return_fig=False):
        """
